...
Twitter.prototype.baseGetRequest = async function (endpointURL, params, headers) {
  ...
  if (!res.body) throw new Error(`Unsuccessful request to ${endpointURL} with ${params}`);

  return res.body;
};

Twitter.prototype.getTweet = async function ({
  tweetId,
  tweetFields = "",
  expansions = "",
  mediaFields = "",
...
Twitter.prototype.searchRecentTweets = async function ({
  query,
  tweetFields = "",
  expansions = "",
  mediaFields = "",
...
Twitter.prototype.oEmbed = async function ({
  url,
  maxwidth = 500,
  align = "none",
  theme = "light",
